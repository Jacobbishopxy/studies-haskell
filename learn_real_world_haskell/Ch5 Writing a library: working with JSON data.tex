...
让我们首先从函数类型开始。它接受起始与结束字符，一个打印某些未知类型\acode{a}值的函数，以及一个类型为\acode{a}的列表，返回一个
类型为\acode{Doc}的值。

注意尽管我们的类型签名提及了四个参数，在函数定义中仅列出了三个。这遵循了简化定义的规则，如\acode{myLength xs = length xs}等同于
\acode{myLength = length}。
