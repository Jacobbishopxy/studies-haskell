...
现在让我们来看一下\acode{getContents}，这是一个从标准输入中读取所有东西直到遇到结束符的 I/O action。其类型为
...
\acode{shortLinesOnly}函数工作如下：接受一个字符串例如\acode{"short\\nlooooooooooooooong\\nshort again"}，该字符串
...
从输入中获取字符串，通过一个函数进行转换，然后再输出的这个模式太常见了，因此存在一个名为\acode{interact}的函数专门用作此模式。
